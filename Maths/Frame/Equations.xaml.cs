...
namespace Maths.Frame
{
    ...
    /// </summary>
    public partial class Equations : Page
    {
        ...
        private void LoadTasks()
        {
            try
            {
                var equations = _context.MathEquations.ToList(); // Получите все задачи из БД
                ExamplesListView.ItemsSource = equations;// Установливаем источник данных для ListBox
        ...
        private void OnAnswerButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            var example = button?.DataContext as MathEquation;
        ...
        private void OnDeleteButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            if (MessageBox.Show("Вы уверены, что хотите удалить этот пример?", "Подтверждение удаления", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
            {
                ...
                using (SqliteConnection connection = new SqliteConnection($"Data Source={dbPath}"))
                {
                    ...
                    using (var command = new SqliteCommand("DELETE FROM MathEquations WHERE Id = @Id", connection))
                    {
                ...
                parentWindow.Frame_W.Navigate(new Equations());
            }
        }
    }
}
